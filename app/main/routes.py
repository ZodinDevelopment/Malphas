...
from app.models import User, Post, Message, Notification, Photo, Video
...
@bp.route("/index")
def index():
    text = current_app.config['INDEX_CONTENT']
...
def parlor():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for("main.parlor"))

    page = request.args.get('page', 1, type=int)
    posts = current_user.followed_posts().paginate(
        page, current_app.config['ITEMS_PER_PAGE'], False
    )
    next_url = url_for("main.parlor", page=posts.next_num) if posts.has_next else None
    prev_url = url_for('main.parlor', page=posts.prev_num) if posts.has_prev else None

    return render_template('parlor.html', title="Parlor", posts=posts.items, form=form, next_url=next_url, prev_url=prev_url)
...
def send_message(recipient):
    ...
    return render_template("send_message.html", recipient=recipient, form=form)
